	...
	for (var npc in gn){
		if (gn.hasOwnProperty(npc)){
			...
			if (closetarget.length > 0 && closetarget[1] > 1){
                ...
                dirToFace = dirToTarget(npc, closetarget[0], parseInt(closetarget[2]), parseInt(closetarget[3]), parseInt(closetarget[4]), parseInt(closetarget[5]));			if (coredata.npcs[npc].dir == dirToFace){
...
function getSurroundings(npc, dist) {
    ...
    for (var player in coredata.players){
    	if (gp.hasOwnProperty(player)) {
    		...
    		if (ppos[0] > orig[0] - dist && ppos[0] < parseInt(orig[0]) + dist && ppos[1] > orig[1] - dist && ppos[1] < parseInt(orig[1]) + dist && gp[player].team !== coredata.npcs[npc].team) {
    ...
    for (var npctar in gn){
    	if (gn.hasOwnProperty(npctar)) {
    		...
    		if (ppos[0] > orig[0] - dist && ppos[0] < parseInt(orig[0]) + dist && ppos[1] > orig[1] - dist && ppos[1] < parseInt(orig[1]) + dist && gn[npctar].team !== coredata.npcs[npc].team) {
    			aa = Math.abs(ppos[0] - orig[0]);
    			bb = Math.abs(ppos[1] - orig[1]);
    			hypot = Math.sqrt( aa*aa + bb*bb );
    			//console.log(hypot);
